...
const Booking =()=>{
    ...
    const [dataProduct, setDataProduct] = useState(JSON.parse(localStorage.getItem('product')))
    ...
    return(
        <Main>            
            ...
            <BookingForm 
                ...
                category={dataProduct?.category}
                title={dataProduct?.title}
                place={`${dataProduct?.address}. ${dataProduct?.city}`} 
                checkIn={verifyCheckIn()}
                checkOut={verifyCheckOut()}   
            />
            
            <Policy
                rules={dataProduct?.policy.descriptionRules}
                securityAndHealth={dataProduct?.policy.descriptionSecurityAndHealth}
                cancellation={dataProduct?.policy.descriptionCancellation}
            />     
        </Main>
    )
}
