...
const SearchBar = () => {
  ...
  const onClickHandler = () => {  
    ...
    if(city && startDate && endDate){
        setEndpoint(`http://localhost:8081/products/searchDates?city=${city}&startDate=${moment(startDate).format("YYYY-MM-DD")}&endDate=${moment(endDate).format("YYYY-MM-DD")}`)
      } else if (city && !startDate && !endDate) {
        setEndpoint(`http://localhost:8081/products/searchCity?city=${city}`)
      }
    }
